...
/// every assertion is about the logic and none about wall-clock timing. It drives
/// a bare `Actuator`: what the loop commands is a level, and how a level becomes a
/// wire command is the vendor session's affair, tested there.
@Suite struct ControlLoopTests {
    ...
    @Test func retriesADroppedWriteRatherThanAssumingItLanded() async throws {
        ...
        actuator.setBusy(false)
        await loop.tick(dt: 1)
        #expect(actuator.steps == [4])
    }
    ...
    @Test func hardStopIsNotRateLimited() async throws {
        let (loop, _) = loop(SafetyLimits(ceiling: 1, riseRatePerSecond: 1,
    ...
    @Test func aTickInFlightWhenAHardStopLandsCannotRestoreItsLevel() async throws {
        ...
        #expect(await loop.appliedLevel == 0)
        let writes = actuator.steps.count
        for _ in 0..<3 { await loop.tick(dt: 1) }
        #expect(actuator.steps.count == writes)
    }
    ...
    @Test func fadesDownWhenTheSensorGoesQuiet() async throws {
        ...
        #expect(actuator.steps == [4, 8, 12, 6, 0])
    }
